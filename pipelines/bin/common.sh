...
commit_finalize_version() {
  run git commit -m "Update versions for release"
...
check_tag_exists() {
  ...
  tag=$(git tag -l | grep "^$version$" || true)
  if [[ ! -z $tag ]]; then
...
build_lang_options=( "--python" "--csharp" "--java" )
...
update_project_options=( "--no-commit" "--snapshot" "${build_lang_options[@]}" "${java_build_tool_options[@]}" )
