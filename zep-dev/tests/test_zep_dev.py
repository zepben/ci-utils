...
from zep_dev.commands.cluster.create import create as cluster_create_command
...
def test_cluster_create_loads_components_from_their_file_path(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    components_path = tmp_path / "components.yaml"
    components_path.write_text("helm_repos: {}\ncluster_components: []\n")
    captured: dict[str, ClusterComponents] = {}
    ...
    monkeypatch.setattr(cluster, "create_cluster", capture_components)

    result = CliRunner().invoke(
        cluster_create_command,
        ["--kind-config", str(kind_config), "--components", str(components_path)],
    )

    assert result.exit_code == 0
    assert captured["components"].source_dir == tmp_path.resolve()
